"""Promote peer comparison to Iceberg with dedup guard.

Same pattern as company_financials and financial_ratios promote: get_catalog,
get_or_create_table, check existing record_ids, append_data.
"""
...
from .schema import PEER_COMPARISON_SCHEMA


def promote_peer_comparison(
    records: list[dict],
    *,
    warehouse_path: str | Path | None = None,
    catalog_path: str | Path | None = None,
) -> dict:
    """Write peer comparison records to Iceberg table.

    Dedup guard: skips record_ids that already exist in the table.
    """
    run_id = emit_start(
        job_name="consumable.peer_comparison",
        input_tables=["base.conformed_facts", "consumable.financial_ratios"],
        output_table="consumable.peer_comparison",
        producer="src/consumable/peer_comparison/promote.py",
    )
    ...
    try:
        ...
        if not records:
            emit_complete(
                run_id=run_id, job_name="consumable.peer_comparison",
                output_table="consumable.peer_comparison",
                producer="src/consumable/peer_comparison/promote.py",
        ...
        table = get_or_create_table(catalog, NAMESPACE, TABLE_NAME, PEER_COMPARISON_SCHEMA)

        # Dedup via DuckDB anti-join (scalable — reads only record_id column)
        records, skipped = filter_existing_records(table, records)
        if skipped:
            print(f"Skipping {skipped} record(s) already in peer_comparison")
